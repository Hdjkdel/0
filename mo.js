...
function renderScoreHud() {
	if (isCasualGame()) {
		...
		scoreNode.innerText = `Skor: ${state.game.score}`;
		scoreNode.style.display = 'block';
		cubeCountNode.style.opacity = 0.65 ;
	}
	cubeCountNode.innerText = `Parçalanan Küpler: ${state.game.cubeCount}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			if (isNewHighScore()) {
				highScoreLblNode.textContent = 'Yeni En Yüksek Skor!';
			} else {
				highScoreLblNode.textContent = `Yüksek Skor: ${formatNumber(getHighScore())}`;
			}
